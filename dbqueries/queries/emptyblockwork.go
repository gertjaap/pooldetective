...
SELECT
	...
	WHERE l.id=5 AND j.observed::date > COALESCE((SELECT max(observed_on) FROM analysis_empty_block_work_daily), '2019-01-01'::date) AND j.observed < NOW()::date AND j.merkle_branches IS NULL
	GROUP BY 
	p.id ,
		po.id ,
		l.id,
